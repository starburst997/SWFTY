...
abstract Popup(Layer) from Layer to Layer {
    ...
    inline function _load(?onComplete:Void->Void, ?onError:Dynamic->Void) {
        File.loadBytes("Popup.swfty", function(bytes) {
    ...
    public static inline function load(?width:Int, ?height:Int, ?bytes:Bytes, ?onComplete:Popup->Void, ?onError:Dynamic->Void):Popup {
